...
def render(base):
    ...
    credentials=''.join('<tr><th scope="row">'+e(c['title'])+'</th><td>'+e(c['provider'])+'</td><td>'+e(c.get('completed','Pending verification'))+'</td><td>'+('<a href="'+e(c['verification'],quote=True)+'">Verify</a>' if c.get('verification') else e(c['evidence']))+'</td></tr>' for c in data.get('credentials',[]))
    base.page('capabilities.html','AI/ML skills and evidence','<section class="hero"><div class="eyebrow">Employer / academic review</div><h1>AI and automation,<br>connected by evidence.</h1><p class="lead">Applied AI, quantitative research, robotics and governed automation share a common engineering discipline: trace inputs, test decisions, record failures and make results reproducible.</p></section><div class="grid">'+cards+'</div><section><h2>Credentials and learning evidence</h2><p>Only artifacts located and read from Drive are listed. A learning artifact is not presented as a professional certification unless its verification evidence supports that claim.</p><div class="table-wrap"><table><thead><tr><th>Credential or evidence</th><th>Provider</th><th>Date</th><th>Verification / source</th></tr></thead><tbody>'+credentials+'</tbody></table></div></section><section><h2>Capability-to-evidence map</h2><p>This is a portfolio review framework, not a university rubric, awarded qualification or certification of professional competence.</p><div class="table-wrap"><table><thead><tr><th>Capability</th><th>Demonstrated or recorded evidence</th><th>Remaining gate</th></tr></thead><tbody>'+rows+'</tbody></table></div></section>')
